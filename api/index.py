...
def ler_qr(slug: str):
    try:
        response = supabase.table("qrcodes").select("*").eq("slug", slug).limit(1).execute()
        if not response.data:
            return "QR Code invalido", 404

        item = response.data[0]
        novo_contador = int(item.get("contador", 0)) + 1
        supabase.table("qrcodes").update({"contador": novo_contador}).eq("slug", slug).execute()
...
def gerar_qr(slug: str):
    try:
        response = supabase.table("qrcodes").select("slug").eq("slug", slug).limit(1).execute()
        if not response.data:
            return "QR Code invalido", 404
